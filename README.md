...
Cloud Run에서 Palworld VM을 제어하는 대시보드입니다. VM 시작·중지, 서버 상태·접속자 조회, 저장·종료, 자동 종료를 지원합니다.
...
flowchart TB
    ...
    Dashboard -->|"VM 조회 · 시작 · 중지"| Compute["Compute Engine API"]
...
| 주체 | 필요한 권한 |
| --- | --- |
| Cloud Run 런타임 서비스 계정 | `compute.instances.get`, `compute.instances.start`, `compute.instances.stop`를 포함한 custom role 또는 `roles/compute.instanceAdmin.v1` |
